...
import {addNetworkDevice, queryNetworkDevice, removeNetworkDevice, updateNetworkDevice} from './service';
...
const TableList = () => {
  ...
  try {
    await updateNetworkDevice(value, current_id);
    hide();
  ...
  const dateFieldList = ["add_time","modify_time"]
  const base_columns = [{
                                  title: 'ID',
                                  dataIndex: 'id',
                                  hideInForm: true,
                              ...
                              render: (text, record) => (
                                <>
                                  ...
                                  <EditOutlined title="编辑" className="icon" onClick={async () => {
                                    record.add_time = moment(record.add_time);record.modify_time = moment(record.modify_time);
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, {selectedRows}) => [
          ...
          <Input.Search style={{marginRight: 20}} placeholder="搜索设备资产 " onSearch={value => {
            setParamState({
              search: value,
            });
        ...
        request={(params, sorter, filter) => queryNetworkDevice({...params, sorter, filter})}
